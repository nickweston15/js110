/*
In the original code, we were trying to concatenate arrays using the +
operator. This works, but it converts the arrays to strings and
concats them. Then we try to call join on strings, which isn't allowed,
you can only call that on arrays. So in order to merge our arrays, instead
of using +, we use the concat method to fix this.
*/
...
let dish = random(ingredients, 4)
  .concat(random(spices, 2))
  .concat(random(extras, 3));
